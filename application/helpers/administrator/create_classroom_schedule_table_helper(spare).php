...
function create_classroom_table($classroom_schedule,$class_id){
	$tf_hours = array("08:00-09:00","09:00-10:00","10:00-11:00","11:00-12:00","12:00-13:00","13:00-14:00","14:00-15:00","15:00-16:00","16:00-17:00","17:00-18:00","18:00-19:00","19:00-20:00");
	if($classroom_schedule!="Schedule is not available for this student."){
	$schedule = new SimpleXMLElement($classroom_schedule);
	
			$result_columns = _xml_num_of_columns_class($schedule);
			...
			$table_result = '<h1 style="text-align: center; text-decoration: underline; margin-top:0px;">'.$class_id.'</h1>
							<div class="table-responsive first-scroller">
							 <table class="table table-bordered " id="classroom_schedule">
			                  <thead>
			                    <tr>
					...
					foreach ($schedule->xpath($xpath_s) as $course) {
						...
						if ($j==1) {
							if (!in_array($cs_id, $courses_on_mon)) {
								...
								$table_result .= '<td class="table-data mon" rowspan="'.$course['duration'].'">'. _collapse_view_class($course->course_title ,$course->official_course_id ,$course->semester ,$course->course_type ,$course->professor_name ,$course->classroom ,$counter_1 ,$counter_2) .'</td>';
						...
						elseif ($j==2) {
							if (!in_array($cs_id, $courses_on_tue)) {
								$table_result .= '<td class="table-data"  rowspan="'.$course['duration'].'">'. _collapse_view_class($course->course_title ,$course->official_course_id ,$course->semester ,$course->course_type ,$course->professor_name ,$course->classroom ,$counter_1 ,$counter_2) .'</td>';
						...
						elseif ($j==3) {
							if (!in_array($cs_id, $courses_on_wed)) {
								$table_result .= '<td class="table-data wed"  rowspan="'.$course['duration'].'">'. _collapse_view_class($course->course_title ,$course->official_course_id ,$course->semester ,$course->course_type ,$course->professor_name ,$course->classroom ,$counter_1 ,$counter_2) .'</td>';
						...
						elseif ($j==5) {
							if (!in_array($cs_id, $courses_on_fri)) {
								$table_result .= '<td class="table-data fri"  rowspan="'.$course['duration'].'">'._collapse_view_class($course->course_title ,$course->official_course_id ,$course->semester ,$course->course_type ,$course->professor_name ,$course->classroom ,$counter_1 ,$counter_2).'</td>';
							...
							<div class="table-responsive first-scroller">
							 <table class="table table-bordered " id="classroom_schedule">
			                  <thead>
			                    <tr>
			                      <th class="table-data-timeframes" id="header_timeframe" style="min-width: 94px; max-width: 94px;"></th>
			                      <th class="table-header mon"   rowspan="1" >ΔΕΥΤΕΡΑ</th>
			                      <th class="table-header"   rowspan="1">ΤΡΙΤΗ</th>
			                      <th class="table-header wed"   rowspan="1">ΤΕΤΑΡΤΗ</th>
			                      <th class="table-header"   rowspan="1">ΠΕΜΠΤΗ</th>
			                      <th class="table-header fri"   rowspan="1">ΠΑΡΑΣΚΕΥΗ</th>
			                      <th class="table-header"  rowspan="1" style="display:none;" id="hidden_column_header"></th>
			                    </tr>
			                  </thead>
			                  <tbody>';
...
function _collapse_view_class($course_title ,$official_course_id ,$semester ,$course_type ,$professor_name ,$classroom ,$counter_1 ,$counter_2) {
					  ...
					  <div class="panel panel-default">
					    ...
					    <div  class="panel-collapse collapse in">
					      <div class="panel-body " >' .
					        	$professor_name . '</br>'.
                                                        translate_cs_type_class($course_type) . '</br>'.
					        	'Εξάμηνο: ' .	$semester . '</br>' .
 ...
 function _xml_num_of_columns_class($schedule) {
	 ...
	 function translate_cs_type_class($cs_type) {
        if ($cs_type=='lab') {
            return 'Εργαστήριο';
        }
        else {
...
/* End of file create_classroom_schedule_table_helper.php */
/* Location: ./application/helpers/administrator/create_classroom_schedule_table_helper.php */