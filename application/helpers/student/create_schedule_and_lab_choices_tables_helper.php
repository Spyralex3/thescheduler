...
function _create_schedule($schedule) {
							 ...
							 <table class="table table-bordered " id="schedule">
			                  <thead>
			                    <tr>
			                      ...
			                      <th class="hidden_column_cell"  rowspan="1" id="hidden_column_header"></th>
			                    </tr>
			                  </thead>
			                  <tbody>';
					...
					foreach ($schedule->xpath($xpath_s) as $course) {
						...
						if ($j==1) {
							if (!in_array($cs_id, $courses_on_mon)) {
								...
								$table_result .= '<td class="table-data mon" rowspan="'.$course['duration'].'">'. _collapse_view($course->course_title ,$course->official_course_id ,$course->semester ,$course->course_type ,$course->professor_name ,$course->classroom ,$course->helper_professor_names ,$counter_1 ,$counter_2) .'</td>';
						...
						elseif ($j==2) {
							if (!in_array($cs_id, $courses_on_tue)) {
								$table_result .= '<td class="table-data"  rowspan="'.$course['duration'].'">'. _collapse_view($course->course_title ,$course->official_course_id ,$course->semester ,$course->course_type ,$course->professor_name ,$course->classroom  ,$course->helper_professor_names ,$counter_1 ,$counter_2) .'</td>';
						...
						elseif ($j==3) {
							if (!in_array($cs_id, $courses_on_wed)) {
								$table_result .= '<td class="table-data wed"  rowspan="'.$course['duration'].'">'. _collapse_view($course->course_title ,$course->official_course_id ,$course->semester ,$course->course_type ,$course->professor_name ,$course->classroom  ,$course->helper_professor_names ,$counter_1 ,$counter_2) .'</td>';
						...
						elseif ($j==5) {
							if (!in_array($cs_id, $courses_on_fri)) {
								$table_result .= '<td class="table-data fri"  rowspan="'.$course['duration'].'">'._collapse_view($course->course_title ,$course->official_course_id ,$course->semester ,$course->course_type ,$course->professor_name ,$course->classroom  ,$course->helper_professor_names ,$counter_1 ,$counter_2).'</td>';
			...
			$table_result .= '<td class="hidden_column_cell"  id="hidden_column_cell"></td>';
			$table_result .= '</tr> ';
		}
			$table_result .= '</tbody>
				                </table>
...
function _xml_num_of_columns($schedule) {
	...
	function _collapse_view($course_title ,$official_course_id ,$semester ,$course_type ,$professor_name ,$classroom  ,$helper_professor_names ,$counter_1 ,$counter_2) {
		$panel_lab_style=($course_type=='lab') ? 'background-color:#679FD2;' : '' ;
		
		$string = '<div class="panel-group" id="'
					. $counter_2 . '">
					  <div class="panel panel-default">
					    <div class="panel-heading " style="word-wrap: break-word; '.$panel_lab_style.'"">
					    ...
					    <div id="' . $counter_1 . '" class="panel-collapse collapse ">
					      <div class="panel-body " >' .
					        	$professor_name . '</br>'.
								 ($helper_professor_names!="" ? $helper_professor_names . '</br>' : '') .
	...
	function _create_lab_choices_table($schedule) {
		include ('tf2daytime_helper.php');

		$option_counter = 0;
		$table_head='';
		$table_body='<tbody> <tr>';
		$table='<form class="form-horizontal" action="' .site_url("student/schedule/course_pending"). '"
	                  role="form" method="post" accept-charset="utf-8" id="course_pending">
	                    <div class="table-responsive scroller">';

		$table.= '<table class="table " border="1" id="table_labs">
					<thead>
						<tr>';

			foreach ($schedule->pending_courses->course as $course) {

				$table_head .='<th class="table-header">'.'<div class="panel-group" id="official_course_id123458 ">
		                                  <div class="panel panel-default ">
		                                    <div class="panel-heading " style="background-color: #679FD2;">
		                                      <h4 class="panel-title">
		                                        <a data-toggle="collapse" data-parent="#official_course_id123458"
		                                         >' . (string)$course->course_title .
		                                         '</a>
			                                      </h4>
			                                    </div>
			                                    <div id="counter_1234569" class="panel-collapse collapse in">
                                                        <div class="panel-body " >'.
                                                                        (string)$course->professor_name . '</br> '.
																		
																		 ($course->helper_professor_names!="" ? $course->helper_professor_names . '</br>' : '') .
                                                                        translate_cs_type((string)$course->course_type) . '</br> '.
                                                                        'Εξάμηνο: '.	(string)$course->semester . '</br> '.
                                                                        'Αίθουσα: ' .	(string)$course->classroom . '</br> '.
                                                                        'Κωδικός: '.     (string)$course->official_course_id .  ' </br>'.
														'</div>
                                                </div>
                                                </div>
                                            </div>
				...
				$table_body	.='<td class="table-data" ><select    name="labs['.$course['cs_id'].']"
				class="form-control">';
				$table_body	.= ' <option value="'.$option_counter.'">' . ' </option>';

				foreach ($course->tf_choices->choice as $choice) {
					if((int)$choice->tf[0] == 100){
						$table_body .=' <option value="'.$course['cs_id'].'_'.$choice['cs_id'].'_'.$choice['tf_option'].'">Χωρίς παρακολούθηση</option>';
					}else{ 
						$tf_count = $choice->tf->count();
						$tf_begin = (int)$choice->tf[0];
						$tf_end = (int)$choice->tf[$tf_count-1];
						$hr_begin = $tf_to_daytime[$tf_begin]['hr_begin'];
						$hr_end = $tf_to_daytime[$tf_end]['hr_end'];
						$day_of_week = $tf_to_daytime[$tf_end]['day_of_week'];
						$table_body .=' <option value="'.$course['cs_id'].'_'.$choice['cs_id'].'_'.$choice['tf_option'].'">' .$day_of_week. ' ' .$hr_begin.'-' .$hr_end. ' </option>';
					}
				}

				$table_body	.='</td></select>';
				$option_counter=0;
			}

			$table.= $table_head . '</thead>'. $table_body . '</tr> </tbody> </table>';
			$table.= '</div></br>
							<div style="width:200px; display:block; margin:auto;" id="submit_div">
		                        <button type="submit" class="btn btn-primary" id="submit" >Καταχώρηση Δηλώσεων</button>
		                      </div>
...
/* End of file create_schedule_and_lab_choices_table_helper.php */
/* Location: ./application/models/create_schedule_and_lab_choices_table_helper.php */	
